...
If the heuristic `displayName` returned by extraction is generic ("L2E_Sample_Invoice – embedded image (page 1)") and YOU can identify what the asset actually is from context — call `set_template_asset_label(assetId, displayName, kind?)` to give it a real name like "L2E round logo". Future searches will find it by that name.
...
pub async fn journal_ingest(
    ...
    // v0.28.71 — one tmpId per journal_ingest call, generated at outer
    // scope so the assistant-done emit (way after the manager loop
    // exits) can reference the same id the manager loop's
    // assistant-message-created + chunk events used.
    ...
    // v0.20.12 — accumulate document ids generated by every run_python /
    // edit_python_artifact call across the entire turn (all manager
    // passes). These get folded into the assistant payload so the chat
    // surface can render a FileCard even when the LLM forgets the
    // doc#N marker in its prose.
    let mut all_generated_doc_ids: Vec<i64> = Vec::new();
    ...
    if let Some(fp) = fast_path_extraction {
        ...
        // v0.28.70/.71 — emit assistant-message-created on the FIRST
        // manager iteration only. The tmpId is scoped to the whole
        // journal_ingest call (declared at outer scope above) so it
        // stays valid through the manager loop AND the eventual
        // assistant-done emit far below.
        if manager_iter == 0 {
            use tauri::Emitter;
            let _ = app.emit(
            ...
            match turn_res {
                ...
                Ok(turn) => {
                    ...
                    for call in turn.tool_calls {
                        ...
                        // v0.20.12 — pluck generatedDocumentIds out of
                        // run_python / edit_python_artifact results so
                        // the file card renders even when the LLM
                        // forgets the doc#N marker in its prose.
                        if matches!(call.name.as_str(), "run_python" | "edit_python_artifact") {
                            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&result) {
                                if let Some(arr) = v.get("generatedDocumentIds").and_then(|x| x.as_array()) {
                                    for item in arr {
                                        if let Some(id) = item.as_i64() {
                                            if !all_generated_doc_ids.contains(&id) {
                                                all_generated_doc_ids.push(id);
                                            }
                                        }
                                    }
                                }
        ...
        .is_empty()
    {
        ...
        if let Some(obj) = combined.as_object_mut() {
            ...
            if !all_generated_doc_ids.is_empty() {
                obj.insert(
                    "generatedDocumentIds".into(),
                    serde_json::json!(all_generated_doc_ids),
                );
            }
        }
        combined.to_string()
    } else {
        // v0.20.12 — fold the accumulated doc ids into the payload so
        // ChatTurn renders FileCards even when the LLM's prose forgets
        // the doc#N marker.
        let mut combined: serde_json::Value =
            serde_json::from_str(&extraction_record).unwrap_or_else(|_| serde_json::json!({}));
        if !all_generated_doc_ids.is_empty() {
